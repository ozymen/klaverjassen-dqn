...
import time
from player.klaverjassen import Klaverjassen
import copy
...
class Mcts:
    ...
    def pick(self, cards, eligableCards, state):
        if len(eligableCards) == 1:
            return eligableCards[0]

        board = Klaverjassen([state], self.playerName, self.round.playerPossibleCardsLeft)

        uct = UCTValues(board)

        uct.history = board.history


        action = uct.get_action()
...
class UCT(object):
    def __init__(self, board, **kwargs):
        ...
        self.calculation_time = float(kwargs.get('time', 2))
        self.max_actions = int(kwargs.get('max_actions', 3000))

        # Exploration constant, increase for more exploratory actions,
        # decrease to prefer actions with known higher win rates.
        self.C = float(kwargs.get('C', 104.5)) # 1.4
    ...
    def run_simulation(self):
        ...
        for t in range(1, self.max_actions + 1):
            ...
            if len(legal) == 0:
                print("****** STATE IMPOSSIBLE *****")
                self.blacklist.append(state)
                break
            elif all((player, S) in stats for p, S in actions_states):
